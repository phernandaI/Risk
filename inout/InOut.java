...
public class InOut {
	...
	public static void printTerritorisAtacables(int[][] tauler, int torn, int [][]veins) {
		// TODO
	        for (int i = 1; i < tauler.length; i++) {
	        	if (tauler[i][0] != torn && (tauler[i][0]==veins[i].length)) {
	                 System.out.println("TERRITORI ATACABLE: ");
	                 System.out.println(DamRISK.territoris[i] + " - " + " Pertanya el jugador: "+nomsJugadors[tauler[i][0]]  + " - " + tauler[i][1]);
